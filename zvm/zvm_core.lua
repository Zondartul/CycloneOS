...
function ZVM:Dyn_WriteOperand(OP,RM)
  if RM == 9 then -- Special case: attempting to write to CS
    self:dbg_DynEmitInterrupt("13","1","Dyn_WriteOperand: attempt to write to CS")
...
function ZVM:Dyn_EmitInterrupt(intNo,intParam)
  ...
  self:Emit("VM.IP = "..(self.PrecompileIP or 0))
  self:Emit("VM.XEIP = "..(self.PrecompileTrueXEIP or 0))
  self:Dyn_Emit("VM:Interrupt("..intNo..","..intParam..", nil, nil, dbg_status)")
  ...
  if self.RQCAP == 1 then
    self:Emit("if VM.MEMRQ > 0 then") -- Extended memory request
      ...
      self:Emit("elseif VM.MEMRQ == 2 then") -- Reading
        ...
        self:Emit("VM:Interrupt(28,VM.LADD,nil,nil,dbg_status)")
      ...
      self:Emit("elseif VM.MEMRQ == 3 then") -- Writing
		...
		self:Emit("VM:Interrupt(29,VM.LADD,nil,nil,dbg_status)")
...
function ZVM:Precompile_Step()
  ...
  if math.floor(self.PrecompileXEIP / 128) ~= self.PrecompilePreviousPage then
    ...
    self:Emit("   (VM.PreviousPage.RunLevel ~= 0) then")
      self:dbg_DynEmitInterrupt("14","self.PrecompileIP","Precompile_Step: walked into a no-exec page")
  ...
  if (not Opcode) or (not RM) then--if self.INTR == 1 then
    self.IF = 1
	local dbg_status = self:dbg_collectStatusInfo("failed to fetch opcode", targetIP)
	self:Interrupt(5,12,nil,nil, dbg_status)
 --self: Interrupt(5,12)
    return
  end
  ...
  if self.OpcodeRunLevel[Opcode] then
    self:Emit("if (VM.PCAP == 1) and (VM.CurrentPage.RunLevel > "..self.OpcodeRunLevel[Opcode]..") then")
      --04.03.2022 make it obvious this instr is privileged
	   --print("zvm: instruction is privileged ("..tostring(Opcode)..")")
	   self:Emit("print(\"zvm: privileged instruction error (instr "..tostring(Opcode)..
					", cur runlevel \"..tostring(VM.CurrentPage.RunLevel)..\")\")")
	  --end 04.03.2022
	  
	  self:dbg_DynEmitInterrupt("13","Opcode", "privileged instruction error")
  ...
  if (not Segment1) or (not Segment2) then--if self.INTR == 1 then
	...
	local dbg_status = self:dbg_collectStatusInfo("failed to fetch segment prefix", targetIP)
	self:Interrupt(5,12,nil,nil, dbg_status)
 --self: Interrupt(5,12)
    return
  end
  ...
  if not self.OperandCount[Opcode] then
    ...
    self:dbg_DynEmitInterrupt("13","1","Precompile_Step: opcode is invalid (probably tried to execute data)")
    ...
    if isFixedSize then
      ...
      if not self.EmitOperandByte[1] then 
	  

	    local dbg_status = self:dbg_collectStatusInfo("failed to fetch immediate value (byte 1)", targetIP)
		self:Interrupt(5,22,nil,nil, dbg_status)	
	 --self: Interrupt(5,22)	
		return 
	  end
      self.EmitOperandByte[2] = self:Precompile_Fetch() or 0
      if not self.EmitOperandByte[2] then 
		
	    local dbg_status = self:dbg_collectStatusInfo("failed to fetch immediate value (byte 2)", targetIP)
		self:Interrupt(5,32,nil,nil, dbg_status)
	 --self: Interrupt(5,32) 
		return 
	  end
      ...
      if self.OperandCount[Opcode] > 0 then
        if self.NeedFetchByteLookup[dRM1] then
          ...
          if not self.EmitOperandByte[1] then 
	        ...
	        local dbg_status = self:dbg_collectStatusInfo("failed to read operand 1 byte", targetIP)
		    self:Interrupt(5,22,nil,nil, dbg_status)
		 --self: Interrupt(5,22) 
		    return
		  end
        ...
        if self.OperandCount[Opcode] > 1 then
          if self.NeedFetchByteLookup[dRM2] then
            ...
            if not self.EmitOperandByte[2] then 
	          ...
	          local dbg_status = self:dbg_collectStatusInfo("failed to read operand 2 byte", targetIP)
		      self:Interrupt(5,32,nil,nil, dbg_status)
		   --self: Interrupt(5,32) 
			  return 
			end
...
function ZVM:Step(overrideSteps,extraEmitFunction)
  ...
  if (self.PCAP == 1) and (self.CurrentPage.Execute == 0) and
	...
	local dbg_status = self:dbg_collectStatusInfo("step: current page is no-exec", targetIP)
	self:Interrupt(14,self.IP,nil,nil, dbg_status)
  ...
  if self.PrecompiledData[instructionXEIP] or overrideSteps then
    ...
    if CLIENT then -- FIXME: hack around crash on PCALL
      ...
      if not status then
	    ...
	    local dbg_status = self:dbg_collectStatusInfo("execute precompiled instruction, exception in pcall, ZVM error", targetIP)
		self:Interrupt(5,1,nil,nil, dbg_status)
     --self: Interrupt(5,1)
      end
    end
    VM = previousVM
