...
ZVM.OpcodeTable[0] = function(self)  --END (STOP)
    ...
    self:dbg_DynEmitInterrupt("2","0","Opcode [STOP] / byte 0 / end of program")
...
ZVM.OpcodeTable[13] = function(self)  --DIV
  ...
  self:Dyn_Emit("if math.abs(OP) < 1e-12 then")
    self:dbg_DynEmitInterrupt("3","0","Opcode [DIV]: division by zero")
...
ZVM.OpcodeTable[28] = function(self)  --SPG
  ...
  self:Dyn_Emit("if VM.CurrentPage.RunLevel <= PAGE.RunLevel then")
    ...
    self:Dyn_EmitInterruptCheck()
  self:Dyn_Emit("else")
    self:dbg_DynEmitInterrupt("11","IDX","Opcode [SPG]: current runlevel is higher than target")
...
ZVM.OpcodeTable[29] = function(self)  --CPG
  ...
  self:Dyn_Emit("else")
    ...
    self:dbg_DynEmitInterrupt("11","IDX","Opcode [CPG]: current runlevel is higher than target")
    self:Dyn_EmitInterrupt("11","IDX")
  self:Dyn_Emit("end")
end
--------------------------------------------------------------------------------
...
ZVM.OpcodeTable[36] = function(self)  --FINV
  ...
  self:Dyn_Emit("if math.abs(OP) < 1e-12 then")
    ...
    self:dbg_DynEmitInterrupt("3","1","Opcode [FINV]: division by zero")
...
ZVM.OpcodeTable[92] = function(self)  --INT
  ...
  self:dbg_DynEmitInterrupt("$1","0","Opcode [INT]: now calling the requested interrupt")
...
ZVM.OpcodeTable[95] = function(self) --ERPG
  ...
  self:Dyn_Emit("else")
    ...
    self:dbg_DynEmitInterrupt("12","0","Opcode [ERPG]: specified page is outside of ROM")
...
ZVM.OpcodeTable[96] = function(self)  --WRPG
  ...
  self:Dyn_Emit("else")
    ...
    self:dbg_DynEmitInterrupt("12","0","Opcode [WRPG]: specified page is outside of ROM")
    --self:Dyn_EmitInterrupt("12","0")
  self:Dyn_Emit("end")
end
ZVM.OpcodeTable[97] = function(self) --RDPG
  ...
  self:Dyn_Emit("else")
    ...
    self:dbg_DynEmitInterrupt("12","0","Opcode [RDPG]: specified page is outside of ROM")
    --self:Dyn_EmitInterrupt("12","0")
  self:Dyn_Emit("end")
end
